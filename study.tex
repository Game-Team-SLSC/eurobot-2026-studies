...
On a :
\begin{itemize}
    \item Epaisseur d'une palette $e_{palette} = 30\,\mathrm{mm}$;
    \item Epaisseur du tapis roulant $e_{tapis} = 1.5\,\mathrm{mm}$;
    \item Hauteur de dent de poulie $h_{dent} = 1.2\,\mathrm{mm}$;
    \item Jeu entre chaque palette $j = 0.5\,\mathrm{mm}$;
\end{itemize}
Calculons la circonference du diamètre exterieur de la poulie au niveau de l'axe. \\
On a $C_{cercletapis} = 2 \cdot (e_{palette} + j) $ \\
Et $C_{cercletapis} = 2 \cdot \pi \cdot r_{cercletapis} $ donc
\begin{align*}
    2 \cdot (e_{palette} + j) &= 2 \cdot \pi \cdot r_{cercletapis} \\
    \Leftrightarrow r_{cercletapis} &= \frac{e_{palette} + j}{\pi}
\end{align*}
\begin{align*}
    r_{out} & = r_{cercletapis} - e_{tapis} \\
    & = \frac{30 + 0.5}{\pi} - 1.5 \\
    & = \boxed{8.2\,\mathrm{mm}}
\end{align*}
\begin{align*}
    r_{in} & = r_{out} - h_{dent} \\
    & = 8.2 - 1.2 \\
    & = \boxed{7.0\,\mathrm{mm}}
\end{align*}

On ajoutera dans la poulie des hinges d'épaisseur $h_{hinge} = 0.3\,\mathrm{mm}$ pour augmenter la durée de vie du tapis et sa capacacité à se plier
Le pitch diameter se trouver alors au niveau du haut de ces hinges :
\begin{align*}
    r_{pitch} & = r_{out} + h_{hinge} \\
    & = 8.2 + 0.3 \\
    & = \boxed{8.5\,\mathrm{mm}}
\end{align*}

\subsection{Poulie entrainée}
Déterminons à présent le nombre de dents de la poulie entrainée. \\
On sait $D_{pitch} = \frac{P \cdot Z}{\pi}$ où $P$ est le pas (distance entre deux dents) et $Z$ le nombre de dents donc $Z = \frac{\pi \cdot D_{pitch}}{P}$. \\

$$Z = \frac{\pi \cdot 2 \cdot 8.5}{5} = 10.7 \approx \boxed{11} \, \mathrm{dents}$$
